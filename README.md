...
All scores below are computed from the committed per-image prediction JSONs in [`evaluation/results/`](evaluation/results/) at the 0.5 decision threshold (no read failures in any run).

**Competition validation set** — balanced accuracy **99.37%**: real images (COCO val2017, 5,000) are classified with **100.00% accuracy** (zero false positives), and AI-generated images (DALL·E Advanced, 8,843) with **98.74%**.

**Robustness study (SID_Set, 38,897 fake / 30,983 real images per condition)** — the clean sets are compared against the six real-world transformations generated by `scripts/batch_transform.py`:

| Transformation | Fake detection (TPR) | Real false positives (FPR) | Balanced accuracy |
|---|---:|---:|---:|
| clean (no transform) | 99.99% | 2.41% | 98.79% |
| JPEG compression | 99.17% | 0.41% | 99.38% |
| Gaussian blur | 99.98% | 3.05% | 98.47% |
| rescaling | 99.56% | 0.71% | 99.43% |
| Gaussian noise | 98.57% | 0.10% | 99.24% |
| color jitter | 99.99% | 0.44% | 99.78% |
| center crop | 99.99% | 1.06% | 99.47% |

Detection of AI-generated images stays above 98.5% under every transformation, and the real-image false-positive rate stays at or below 3.05% — clean-set performance is essentially preserved under all six real-world transformations. JPEG compression and Gaussian noise are the hardest for fake detection; Gaussian blur costs the most real-image false positives.

## Error Analysis

The full error-analysis report — representative false positives and false negatives, residual-heatmap discussions, and robustness-vs-false-positive trade-offs — is in [`analysis/MIRROR_Error_Analysis.docx`](analysis/MIRROR_Error_Analysis.docx). Supporting per-class dataset statistics for the validation set and SID_Set are in [`analysis/Statistics/`](analysis/Statistics/).

## Demo Video

Watch the end-to-end demonstration (inference, robustness evaluation and error-analysis tooling) on YouTube: **[Robust AI-Generated Image Detection — Demo](https://youtu.be/9bcmF4S9U0U)** (public; also linked from the Devpost submission).
...
```text
robust-aigc-detector/
├── analysis/               # error-analysis report and dataset statistics
...
│   └── results/            # committed per-image prediction scores (validation set + SID_Set robustness study)
├── scripts/
